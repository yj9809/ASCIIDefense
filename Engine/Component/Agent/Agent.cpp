...
namespace Wanted
{
	...
	bool Agent::FindPath(const Vector2& start, const Vector2& target, const std::vector<std::vector<int>>& grid, std::vector<Vector2>& outPath)
	{
		ClearLists();
	...
	bool Agent::IsGridRange(const Vector2& position, const std::vector<std::vector<int>>& grid) const
	{
	...
	float Agent::GetHCost(const Vector2& position, const Vector2& target) const
	{
	...
	bool Agent::IsCloseToTarget(const Vector2& position, float cost) const
	{
	...
	std::vector<Vector2> Agent::ConstructPath(Node* target)
	{
